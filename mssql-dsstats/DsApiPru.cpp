...
// * By          Date     Description                                    *
// * ----------- -------- ---------------------------------------------- *
// * Mohan Palat 20141209 For ODBC Log Scan included SQL Server and DB2  *
// *                      DSAPI Pru Version - 3.5 to 3.6                 *
// * Mohan Palat 20161209 Created Version 3.6U for Linux Migration       *
// * Mohan Palat 20170626 Changed \\ (DOS) to / UNIX Convension - Folder *
// ***********************************************************************
//

// Changes Made for SQL Server
//     GetUniqueTimeStamp - SQL Changed to use SYSDATETIME() instead of CURRENT TIMESTAMP
//     WITH UR in the insert update removed
...
bool DsApiPru::GetUniqueLoadId(void)
{
    ...
    d1->ClearSql();
    // Changed for MS SQL Server
    d1->SetSql("SELECT LEFT(REPLACE(REPLACE(REPLACE(REPLACE(CONVERT(CHAR(27), SYSDATETIME())");
    d1->SetSql(" ,':',''),'.',''),'-',''),' ',''),19)");
    if (d1->Exec())
        return true;
